...
exposes the CloudCenmax `/api/v1` REST API as MCP tools, so AI assistants can
manage cloud resources, billing, and the catalog.
...
Env vars:
- `CLOUDCENMAX_API_BASE` — platform base URL, no trailing slash (default `http://127.0.0.1:8000`). The server appends `/api/v1`.
...
Issue a key via the dashboard `/api-keys` page or in `php artisan tinker`:

```php
[$key, $token] = App\Models\ApiKey::issue($org, 'mcp', null, null, ['read', 'write']);
